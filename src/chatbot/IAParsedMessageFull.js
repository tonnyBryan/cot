...
export default function IAParsedMessageFull({ text, animate = false }) {
    const lines = parseMarkdown(text);
    const animatedValues = useRef(lines.map(() => new Animated.Value(0))).current;

    useEffect(() => {
        if (animate) {
            // Animation enchaînée ligne par ligne
            ...
            // Démarre les animations séquentiellement
            Animated.stagger(100, animations).start();
        } else {
            animatedValues.forEach((v) => v.setValue(1));
        }
    }, [text, animate]);
    ...
    return (
        <View>
            {lines.map((line, index) => {
                let baseStyle = { fontSize: 15, color: '#333', marginBottom: 4 };

                if (line.type === 'remark') {
                    baseStyle = { fontSize: 14, color: '#666', fontStyle: 'italic', marginTop: 8 };
                } else if (line.type === 'heading') {
                    baseStyle = { fontSize: 17, fontWeight: 'bold', marginTop: 12, marginBottom: 6, color: '#222' };
                }
